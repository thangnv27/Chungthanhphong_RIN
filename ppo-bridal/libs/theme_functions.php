<?php
if (!defined('DEV_LOGO'))
    define('DEV_LOGO', get_template_directory_uri() . "/images/dev-logo.png");
if (!defined('DEV_LINK'))
    define('DEV_LINK', "http://the-huynh.com/");
...
if (is_admin() and !function_exists("ppo_update_admin_footer")) {
    ...
    function ppo_update_admin_footer() {
        ...
        $text = __('<img src="' . DEV_LOGO . '" width="24" />Hệ thống CMS phát triển bởi <a href="' . DEV_LINK . '" title="Xây dựng và phát triển ứng dụng">THE-HUYNH.COM</a>.');
        echo $text;
    }
...
function get_image_url($show = true, $size = 'thumbnail') {
...
function getpagenavi($arg = null) {
    ...
    <div class="paging">
        ...
        ?>
            <div class="inline"><?php previous_posts_link('« Previous') ?></div><div class="inline"><?php next_posts_link('Next »') ?></div>
        <?php } ?>
    </div>
    <?php
}
...
HTML;
    ...
    $bill_html .= <<<HTML
                    <tr>
                        ...
                        <td align="right" style="text-align: right;">{$totalPayFormat} đ</td>
                        <td></td>
                    </tr>
                </tbody>
            </table>
...
function get_post_categories(){
        global $product;
        $product_categories = wp_get_post_terms(get_the_ID($product),'portfolio_category');
